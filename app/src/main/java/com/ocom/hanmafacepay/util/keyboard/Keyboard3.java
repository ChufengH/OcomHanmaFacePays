...
public class Keyboard3 implements Listener {
    private static Keyboard3 instance = null;
    ...
    private IKeyboardListener3 listener = null;
    ...
    private Keyboard3(UsbSerialDriver driver, Context context) {
        this.context = context;
        this.driver = driver;
        this.device = driver.getDevice();
        this.timer = new Timer();
        this.listener = new DefaultKeyboardListener3();
        this.setLayout(0);
    }
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if ("com.geekmaker.USB_PERMISSION".equals(action)) {
                    ...
                    if (intent.getBooleanExtra("permission", false)) {
                        if (device != null) {
                            Keyboard3.this.initPort();
                        } else {
                            Keyboard3.this.listener.onKeybroadException(new Exception("no permisson"));
                        }
                    } else {
                        Keyboard3.this.listener.onKeybroadException(new Exception("no permisson"));
                    }
    ...
    private void initPort() {
        ...
        try {
            ...
            this.listener.onKeybroadException(var4);
            return;
        }
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if ("android.hardware.usb.action.USB_DEVICE_DETACHED".equals(action)) {
                    ...
                    if (device != null) {
                        ...
                        Keyboard3.this.release();
                        Keyboard3.this.listener.onKeybroadRelease();
                    }
                }
        ...
        this.listener.onKeybroadAvailable();
    }
    ...
    public void sendTips(String tip) {
        this.lastTip = tip;
        if (tip.length() > 8) {
            tip = tip.substring(0, 8);
        }
    ...
    public void sendNumber(String string, boolean remember) {
        this.updateDisplay(string, remember);
    }
    ...
    public static void toast(Context context, String msg) {
        if (context instanceof Activity) {
            ((Activity) context).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ToastUtil.INSTANCE.showLongToastCenter(msg);
                }
            });
        }
    }

    public static Keyboard3 getInstance() {
        return instance;
    }

    private static Keyboard3 create(Context context) {
        ...
        if (driver == null) {
            ...
            return new Keyboard3(driver, context);
        }
    }

    public static Keyboard3 get(Context context) {
//        synchronized (Keyboard3.class) {
//            if (instance == null)
...
//        }
    ...
    public void setListener(IKeyboardListener3 listener) {
            ...
            while (buffer.remaining() > 0) {
                ...
                if (isResponse) {
                    ...
                    if (this.listener != null) {
                        if (resp[0] != 1) {
                            this.listener.onKeybroadKeyUp(keyCode, keyName);
                        } else {
                        /*    if (this.displayString.length() < 8 && (keyName.matches("^\\d$") || keyName.equals("."))) {
                                ...
                                if (pay > 0.0D) {
                                    ...
                                    this.listener.onPay(new IPayRequest() {
                                        ...
                                        public void setResult(boolean isOk) {
                                            Keyboard3.this.lock = false;
                                            Keyboard3.this.lastDisplay = "0";
                                            Keyboard3.this.displayString = new StringBuffer();
                                            if (isOk) {
                                                Keyboard3.this.showTip("success");
                                            } else {
                                                Keyboard3.this.showTip("fail");
                                            }
                            ...
                            this.listener.onKeybroadKeyDown(keyCode, keyName);
                        }
                    }
                }
            }
    ...
    private void calculate() {
        if (this.num1 != null && this.num2 != null && this.op.length() > 0) {
            ...
            if (result > 9.9999999E7D) {
                this.reset(false);
                this.showTip("huge");
                return;
            }

            this.num1 = (new BigDecimal(result)).toPlainString();
    ...
    public int sendRequest(byte[] data, byte type) {
    ...
    public synchronized int sendRequest(byte[] data, byte type, IResponse response) {
        if (this.serialManager == null) return -1;
        ...
        this.timer.schedule(this.lastUpdateTask, 200L);
        return seq;
    }
    ...
    public void onRunError(Exception e) {
        this.listener.onKeybroadException(e);
    }
    ...
    private class RetryTask extends TimerTask {
        ...
        public void run() {
            ...
            Keyboard3.this.requests.clear();
            Keyboard3.this.sendRequest(this.data, this.type);
        }
    }
}
